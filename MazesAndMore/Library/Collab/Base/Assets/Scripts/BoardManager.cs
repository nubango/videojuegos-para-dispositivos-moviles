...
namespace MazesAndMore
{
    ...
    public class BoardManager : MonoBehaviour
    {
        ...
        public Vector2 GetNextPosition(Vector2 position, Vector2 newDir)
        {
            ...
            if (positionsNextIntersection.Count > 0)
            {
                ...
                if (position == positionsNextIntersection[0])
                {
                    ...
                    /*
                     ||
                        (map.tiles[(int)position.y, (int)position.x].ice &&
                        ((positionsNextIntersection[0] - position).normalized.x != (position - removePos).normalized.x ||
                        (positionsNextIntersection[0] - position).normalized.y != (position - removePos).normalized.y) || 
                        (positionHistory.Count > 1 && 
                        ((positionHistory[positionHistory.Count - 2] - position).normalized.x != (position - removePos).normalized.x ||
                        (positionHistory[positionHistory.Count - 2] - position).normalized.y != (position - removePos).normalized.y)))
                     */
                    if (positionsNextIntersection.Count == 0)
                    {
                        goBack = false;
        ...
        //        // si no hay interseccio y delante no hay pared
        //        while (wallCount(nextPos) == 2)
        //        {
        //            if (isWall(nextPos, currentDir))
        //            {
        //                // cambio de direccion porque en la que vamos hay un muro
